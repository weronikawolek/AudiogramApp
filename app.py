...
class AudiogramApp(ctk.CTk):
    def __init__(self):
        ...
        self.inst_button = ctk.CTkButton(self.top_frame, text="Instruction", command=self.displayInstructions)
        self.inst_button.grid(row=0, column=2, padx=10, pady=5, sticky="ew")

        self.restart_button = ctk.CTkButton(self.top_frame, text="Restart", command=self.restartTest)
        self.restart_button.grid(row=0, column=3, padx=10, pady=5, sticky="ew")
    ...
    def displayInstructions(self):
        ...
        instructions_textbox = ctk.CTkTextbox(instructions_window, width=350, height=200)
        instructions_textbox.pack(padx=10, pady=10, fill=ctk.BOTH, expand=True)

        instructions_textbox.insert("0.0", "Instruction: "
                                           "\n\n1. Choose technician."
                                           "\n2. Choose calibration mode."
                                           "\n3. Enter patient data."
                                           "\n4. Press Start button."
                                           "\n5. Initialize the test on the audiometer."
                                           "\n5a. Patient is going to hear sounds of different frequency and varied intensivity."
                                           "\n5b. When hearing the sound for the first time on determined frequency, patients has to press the button on the audiometer to register hearing threshold."
                                           "\n5c. Pressing the button is going to switch the frequency so the intensivity is going to be moderately increased again."
                                           "\n5d. When the test of left ear is finished, test for right ear will be initialized. After that, test for both ears will be repeated."
                                           "\n6. After generating results, press Save button"
                                           "\n7. Press the Restart button to start another test."
                                           "\n8. To display the previous results, press the Open button."
                                    "\n\nNote: It is important to perform the test in the quite and isolated location in order to obtain the most accurate results.")
        instructions_textbox.configure(state="disabled")

        # Dodanie przycisku zamknięcia okna (zamiast standardowego przycisku okna)
        close_button = ctk.CTkButton(instructions_window, text="Close", command=instructions_window.destroy)
    ...
    def saveAudiogram(self):
        ...
        if file_path:
            try:
                ...
                # Zapis danych pacjenta i audiogramu do CSV
        ...
        if file_path:
            try:
                with open(file_path, mode='r', encoding='utf-8') as csvfile:
                    ...
                    # Wyświetlanie danych pacjenta w zakładce "Wynik"
                    result_text = f"Patient Information:\nName: {patient_data.get('Firstname', '')} {patient_data.get('Lastname', '')}\nAge: {patient_data.get('Age', '')}\nDate: {patient_data.get('Date', '')}\n\n"

                    result_text += "Audiogram Results:\n\n"
                    result_text += "Left Ear:\n"
                    for freq, value in zip(self.frequencies, audiogram_data['Left Ear']):
                        result_text += f"  {freq} Hz: {value} dB\n"

                    result_text += "\nRight Ear:\n"
                    for freq, value in zip(self.frequencies, audiogram_data['Right Ear']):
                        result_text += f"  {freq} Hz: {value} dB\n"

                    self.result_textbox.delete("1.0", ctk.END)
                    self.result_textbox.insert(ctk.END, result_text)

                    if len(audiogram_data['Left Ear']) == len(self.frequencies) and len(
                            audiogram_data['Right Ear']) == len(self.frequencies):
