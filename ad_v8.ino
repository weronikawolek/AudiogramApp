// Impedancja słuchawek JBL Tune 110 - 16 Ohm
  ...
  if (startFlag) {
    ...
    if (volNew != vol) {
      ...
      if (calibrationMode == 50) {
        calibration(dbValue, 4.5, ear3aCalibration);
      } 
      if (calibrationMode == 51) {
        calibration(dbValue, 4.5, tdh39Calibration);
      }
      if (calibrationMode == 52) {
        calibration(dbValue, 4.5, bioCalibration);
      }
    }
  } 
}
